...
struct contact_tree* create_contact_tree(const char* property) {
	...
	else if(!strcmp(property,"date_of_birth")){
...
struct User* find_User_tree(const char* surname,const char* date_of_birth,const char* email,const char* phone,struct contact_tree* tree){
	...
	User* user_to_find = createUser("",surname,date_of_birth,email,phone,"");

	while(tree != NULL){

		if ( !strcmp(tree->user->surname,surname)
		&& !strcmp(tree->user->date_of_birth,date_of_birth)
		&& !strcmp(tree->user->email,email)
		&& !strcmp(tree->user->phone,phone)) {

			deleteUser(user_to_find);
            return tree->user;

		}
		if ( ( *(tree->property_pointer) )(tree->user,user_to_find) > 0){
