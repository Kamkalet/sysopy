...
void* library = dlopen("libshared.so", RTLD_LAZY);
    ...
    struct contact_list* (*add_user)(struct Person* user,struct contact_list* list);
    add_user = dlsym(library, "add_user");

    struct User* (*find_user)(const char* surname,const char* birth,const char* email,const char* phone,struct contact_list* list);
    find_user = dlsym(library, "find_user");

    struct delete_user* (*delete_user)(struct Person* user,struct contact_list* list);
    ...
    struct contact_tree* (*create_contact_book)(const char* value);
    create_contact_book = dlsym(library, "create_contact_book");

    struct add_user_to_tree* (*add_user_to_tree(struct Person* somebody,struct contact_tree* tree);
    add_user_to_tree = dlsym(library, "add_user_to_tree");

/*
    struct User* (*getPersonT)(const char* surname,const char* date_of_birth,const char* email,const char* phone,struct ContactsT* book);
    getPersonT = dlsym(library, "getPersonT");*/

    struct contact_tree* (*delete_user_from_tree)(struct User* user, contact_tree* tree);
    ...
    struct contact_tree* (*sort_tree)(const char* property,struct contact_tree* tree);
    ...
    sb1 = find_User("Kamil","Kaleta","1903-01-01","kamkalet@gmail.com","889278400",l);
	checkTimes(&prevTimes, &prevReal, &firstTimes, &firstReal);

	printf("Finding oldest contact in the list. \n");
    sb2 = find_User("Kamil","Kaleta2","2100-04-12","fullbringer@gmail.com","612345653",l);
	checkTimes(&prevTimes, &prevReal, &firstTimes, &firstReal);
